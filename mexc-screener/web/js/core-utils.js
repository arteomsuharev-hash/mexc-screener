...
(function (root, factory) {
  ...
  // Верхняя граница на то, как далеко назад volHistory/volVolHistory сканируют буфер (audit fix,
  // см. ниже). Без неё цикл идёт от t ПЕРВОЙ сделки в буфере до now с шагом 30с — для активно
  // торгуемого символа буфер естественно "молодой" (cap на кол-во сделок вытесняет старые), но для
  // редко торгуемого (ровно целевые неликвидные монеты Pattern Engine) первая сделка в буфере может
  // быть недели/месяцы в прошлом при том же малом количестве сделок — измерено: 7 дней истории
  // стоили ~107мс на один символ, 30 дней — ~133мс, ЧИСТО от размера этого цикла. Час — тот же
  // порядок величины, что и recentWindowMs/baselineWindowMs у Pattern Engine (минуты-десятки минут),
  // и адаптивность сохраняется: baseline всё так же считается из РЕАЛЬНЫХ последних сделок символа,
  // просто не глубже этого предела.
  const FEATURE_HISTORY_LOOKBACK_MS = 60 * 60000;
  function computeFeatures(trades, depthSnapshots, now) {
    ...
    // по буферу этой монеты, но не глубже FEATURE_HISTORY_LOOKBACK_MS назад (audit fix — см. её
    // определение выше: без этой границы цикл ниже мог идти месяцы вглубь для редко торгуемых
    // символов). Если буфер моложе лимита — bufStart, как и раньше, это просто первая сделка буфера,
    // поведение не меняется для типичного (активно торгуемого) случая.
    const bufStart = Math.max(trades[0].t, now - FEATURE_HISTORY_LOOKBACK_MS);
    const volHistory = [];
