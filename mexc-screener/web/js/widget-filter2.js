...
(function (global) {
  ...
  function qualityScore(candidate, symbol) {
    // Базовый rawScore детектора (0-100, но обычно 40-100) + бонус за ликвидность/подтверждение
    // потоком (п.24-25 ТЗ) — здесь без look-ahead, всё из уже посчитанных candidate.evidence/OFI.
    let score = clamp(candidate.rawScore, 0, 100);
    const coin = global.mexcCoinMap.get(symbol);
    if (coin && coin.vol24 > CFG.minVol24Usd * 4) score += 3; // явно ликвидная монета — небольшой бонус доверия
